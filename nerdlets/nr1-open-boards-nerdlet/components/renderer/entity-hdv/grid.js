...
class HexagonGrid extends Component {
  ...
  updateDimensions(props) {
    const { gridWidth, gridHeight, hexagonLength } = this.state;
    if (
      gridHeight !== props.gridHeight ||
      gridWidth !== props.gridWidth ||
      hexagonLength !== props.hexagons.length
    ) {
      this.setState(
        {
          gridHeight: props.gridHeight,
    ...
    return (
      ...
      >
        {Array.from(Array(rows).keys()).map(row => {
          ...
          const columns =
            remaining < this.state.columns ? remaining : this.state.columns;
          ...
          return (
            <svg
              ...
              y={rowDim.y}
            >
              {Array.from(Array(columns).keys()).map((col, i) => {
                ...
                return (
                  <Popup
                    ...
                    content={this.renderContent(hexagon)}
                  />
                );
              })}
            </svg>
